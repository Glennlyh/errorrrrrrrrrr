...
// Check if freightTime is within acceptable arrival window for cargoDeadline
// Acceptable window: [cargoDeadline - 15 minutes, cargoDeadline]
// Times are in HHMM format (e.g., 1400 = 2:00 PM)
bool isWithinArrivalWindow(int freightTime, int cargoDeadline)
{
    // Validate both times
    if (!isValidTime(freightTime) || !isValidTime(cargoDeadline)) {
        return false;
    }
